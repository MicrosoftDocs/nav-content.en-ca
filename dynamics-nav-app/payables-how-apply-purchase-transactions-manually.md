...
> [!IMPORTANT]  
>   When you apply entries in different currencies to one another, the entries are converted to CAD. Even though the exchange rates for the two relevant currencies are fixed, for example between CAD and EUR, there may be a small residual amount when these foreign-currency amounts are converted to CAD. These small residual amounts are posted as gains and losses to the account specified in the **Realized Gains Account** or **Realized Losses Account** field in the **Currencies** window. The **Amount (CAD)** field is also adjusted on the relevant vendor ledger entries.
